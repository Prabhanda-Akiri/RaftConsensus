...
// for leader to add a new block
func (cfg *config) AddNewBlock() int {
	for iters := 0; iters < 10; iters++ {
		ms := 450 + (rand.Int63() % 100)
		time.Sleep(time.Duration(ms) * time.Millisecond)
		...
		if len(leaders) != 0 {
			rf := cfg.rafts[leaders[lastTermWithLeader][0]]
			index := rf.raftLog.GetDataIndex() + 1
			rf.proposeNew(nil,index,leaders[lastTermWithLeader][0])
			fmt.Printf("\nLeader:	%d, Current term: %d , Log Entries:	%d\n",leaders[lastTermWithLeader][0],rf.term, len(rf.raftLog.Entries))
			return leaders[lastTermWithLeader][0]
		}
	}
	cfg.t.Fatalf("expected one leader, got none")
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		if ok {
			...
			count += 1
			cmd = cmd1
			//fmt.Printf("Get a cmd %d in server %d, index: %d\n", cmd1, i, index)
		}
	}
	return count, cmd
}
